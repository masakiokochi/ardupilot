...
bool ModeLatGuided::_enter()
{
    gcs().send_text(MAV_SEVERITY_INFO, "Entered LatGuided Mode");
    // set desired location to reasonable stopping point
    if (!g2.wp_nav.set_desired_location_to_stopping_location()) {
        return false;
    }
    _latguided_mode = Guided_WP;
...
void ModeLatGuided::update()
{
    switch (_latguided_mode) {
        case Guided_WP:
        {
...
float ModeLatGuided::get_distance_to_destination() const
{
    switch (_latguided_mode) {
    case Guided_WP:
...
bool ModeLatGuided::reached_destination() const
{
    switch (_latguided_mode) {
    case Guided_WP:
...
bool ModeLatGuided::get_desired_location(Location& destination) const
{
    switch (_latguided_mode) {
    case Guided_WP:
...
bool ModeLatGuided::set_desired_location(const struct Location& destination,
                                      float next_leg_bearing_cd)
{
    if (g2.wp_nav.set_desired_location(destination, next_leg_bearing_cd)) {

        // handle guided specific initialisation and logging
        _latguided_mode = ModeLatGuided::Guided_WP;
        sent_notification = false;
        rover.Log_Write_GuidedTarget(_latguided_mode, Vector3f(destination.lat, destination.lng, 0), Vector3f(g2.wp_nav.get_desired_speed(), 0.0f, 0.0f));
        return true;
    }
    return false;
}
...
void ModeLatGuided::set_desired_heading_and_speed(float yaw_angle_cd, float target_speed)
{
    // call parent
    Mode::set_desired_heading_and_speed(yaw_angle_cd, target_speed);

    // handle guided specific initialisation and logging
    _latguided_mode = ModeLatGuided::Guided_HeadingAndSpeed;
    ...
    // log new target
    rover.Log_Write_GuidedTarget(_latguided_mode, Vector3f(_desired_yaw_cd, 0.0f, 0.0f), Vector3f(_desired_speed, 0.0f, 0.0f));
}

void ModeLatGuided::set_desired_heading_delta_and_speed(float yaw_delta_cd, float target_speed)
{
    // handle initialisation
    if (_latguided_mode != ModeLatGuided::Guided_HeadingAndSpeed) {
        _latguided_mode = ModeLatGuided::Guided_HeadingAndSpeed;
        _desired_yaw_cd = ahrs.yaw_sensor;
    }
...
void ModeLatGuided::set_desired_turn_rate_and_speed(float turn_rate_cds, float target_speed)
{
    // handle initialisation
    _latguided_mode = ModeLatGuided::Guided_TurnRateAndSpeed;
    _des_att_time_ms = AP_HAL::millis();
    _reached_destination = false;
    ...
    // log new target
    rover.Log_Write_GuidedTarget(_latguided_mode, Vector3f(_desired_yaw_rate_cds, 0.0f, 0.0f), Vector3f(_desired_speed, 0.0f, 0.0f));
}

bool ModeLatGuided::start_loiter()
{
    if (rover.mode_loiter.enter()) {
        _latguided_mode = Guided_Loiter;
        return true;
    }
    return false;
}
...
void ModeLatGuided::limit_set(uint32_t timeout_ms, float horiz_max)
{
...
void ModeLatGuided::limit_clear()
{
    limit.timeout_ms = 0;
...
void ModeLatGuided::limit_init_time_and_location()
{
...
bool ModeLatGuided::limit_breached() const
{
